...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ score, totalQuestions, onRestart }) => {
    ...
    return (
        <div className="relative text-center text-slate-800 flex flex-col items-center animate-fade-in z-20">
            ...
            <h2 className={`text-4xl md:text-5xl font-black ${messageColor} drop-shadow-lg mb-4`} style={{ textShadow: '2px 2px 0px rgba(0,0,0,0.1)' }}>
                {message}
            </h2>
            <div className="bg-[#FEFBF6] p-6 rounded-3xl shadow-xl w-full max-w-sm mb-8 border-8 border-[#AD8154]/50">
                <p className="text-2xl font-bold mb-2 text-[#232946]">نتيجتك النهائية هي</p>
                <p className="text-7xl font-black text-[#232946]" style={{ textShadow: '4px 4px 0px rgba(0,0,0,0.1)' }}>
